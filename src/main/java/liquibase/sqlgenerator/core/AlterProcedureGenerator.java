...
public class AlterProcedureGenerator extends AbstractSqlGenerator<AlterProcedureStatement> {
    ...
    @Override
    public ValidationErrors validate(AlterProcedureStatement statement, Database database, SqlGeneratorChain<AlterProcedureStatement> sqlGeneratorChain) {
    ...
    @Override
    public Sql[] generateSql(AlterProcedureStatement statement, Database database, SqlGeneratorChain<AlterProcedureStatement> sqlGeneratorChain) {
        List<Sql> sql = new ArrayList<>();

        String schemaName = statement.getSchemaName();
        if (database instanceof MySQLDatabase) {
            sql.add(new UnparsedSql("DROP PROCEDURE IF EXISTS `" + schemaName + "`.`" + statement.getProcedureName() + "`")); //先删除原有的存储过程
        }
